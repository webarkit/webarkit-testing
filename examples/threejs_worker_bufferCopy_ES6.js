...
function start(markerUrl, video, input_width, input_height, render_update, track_update) {
  ...
  document.body.appendChild(canvas_process);
  ...
  const load = function () {
    ...
    worker = new Worker('./worker_bufferCopy_threejs.js')
  ...
  const process = function () {
    ...
    if (!bufferCopy) {
      bufferCopy = imageData.data.buffer.slice();
    ...
    worker.postMessage({ type: 'process', imagedata: data }, [bufferCopy]);
  }
